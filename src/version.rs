...
//! ## Provenance
//!
//! This module is borrowed verbatim from `mediacast-netcatalog` because
//! the SemVer-with-quirks shape generalizes across both domains: switch
//! firmware (Cisco IOS-XE 17.6.4, Aruba AOS-CX FL.10.13.1000) and
//! display firmware (LG webOS 5.5.0 dotted SemVer; Samsung Tizen with
//! family prefixes like `T-KTM2DEUC` followed by build digits; Sony
//! Bravia `PKG6.2940.0030NAA` style strings; PJLink advertises a
//! single revision integer).
//!
//! The Aruba `FL./GL./LL.` constants below are dead code for the
//! display domain but harmless — kept verbatim to preserve byte-for-byte
//! parity with `mediacast-netcatalog`'s matcher so any future fix flows
//! to both crates without divergence.
//!
//! TODO(v0.2): validate display-vendor version-string corners against
//! real catalog data once the doc-crawl effort lands. Likely additions:
//! - Samsung Tizen / SSSP family prefix stripping (`T-KTM*`, `T-MST*`)
//! - Sony Bravia `PKG` prefix + region/codec suffix (`NAA`, `EUA`)
//! - PJLink class-revision dual-component versioning
//!
