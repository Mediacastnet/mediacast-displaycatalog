//! PyO3 bindings — exposed as the `mediacast_displaycatalog._native` extension.
//!
//! The Python-side `mediacast_displaycatalog/` package re-exports these
//! into a more idiomatic surface; this file is the thin FFI seam.
...
#[pyclass(name = "Catalog", module = "mediacast_displaycatalog._native")]
struct PyCatalog {
    inner: RustCatalog,
}
...
impl PyCatalog {
    ...
    /// Look up `(vendor, firmware, command)`. Returns the CLI/payload string
    /// or raises ValueError if the entry is missing / NOT_SUPPORTED.
...
/// Module entry point — `import mediacast_displaycatalog._native`.
#[pymodule]
